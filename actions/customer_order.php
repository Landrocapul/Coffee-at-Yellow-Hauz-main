...
try {
    ...
    $orderStmt = $pdo->prepare("INSERT INTO orders (order_number, table_id, customer_name, order_type, payment_method, subtotal, tax_rate, tax_amount, total_amount, discount_amount, cashier_id, status) VALUES (?, ?, ?, ?, 'cash', ?, ?, ?, ?, 0, ?, 'pending')");
    $orderStmt->execute([$orderNumber, $tableId, $customerName, $orderType, $subtotal, $taxRate, $taxAmount, $totalAmount, $onlineUser['id']]);
